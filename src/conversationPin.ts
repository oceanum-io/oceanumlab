 ...
 * conversation: switching tabs does not move it, and a pinned notebook that
 * has been closed is opened again when it is next needed. The active tab
 * rather than the tracker's current notebook, which is the LAST-FOCUSED
 * notebook even with a terminal or a text file in front.
 */
export class ConversationPin {
   ...
   * cannot be -- deleted, or renamed while closed -- a new notebook takes its
  ...
  notebook(): Promise<NotebookPanel | null> {
    return this._serial(async () => {
      ...
      if (this._panel && !this._panel.isDisposed) {
        // Follows a rename while the notebook is open.
        this._path = this._panel.context.path;
        return this._panel;
      }
  ...
  name(): string | null {
    if (this._panel && !this._panel.isDisposed) {
      return this._panel.title.label;
    }
